function [x_vals, fun_evals] = ds_method(x0, typx, tols, fun, max_iters)
%
% [x_vals, fun_evals] = ds_method(x0, tol, fun, max_iters)
%
% Input:
...
%   approximation to the derivatives, together with the reduction condition
%   from Dennis and Schnabel (page 26) until the tolerances are violated or
...
%   typx = 1;
%   [x_vals, fun_evals] = ds_method(x0, typx, tols, fun, max_iters)
...
while ((iterations<max_iters)& ...
  ...
  % Check if this point is acceptable:
  while ((iterations<max_iters)& ...
         (abs(fx_new)>tol1) & ...
         (abs(f_der)>tol2)  & ...
         (abs(x-x_prev)/max(typx,abs(x)) > tol3) & ...
         (abs(fx_new) > abs(fx_prev))) % No reduction! Move to midpoint.
